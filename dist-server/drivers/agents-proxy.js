...
//   GBS_HARNESS_URL  base URL of the harness (http://127.0.0.1:8899)
...
const TOOLS = [
    {
        name: "list_bots",
        description: "List the other bots (agents) in this Gerer Build Studio workspace you can message, with their model and whether they're busy. Call this before ask_bot to discover who's available.",
    ...
    {
        name: "list_tasks",
        description: "List work on the shared task board. Use filters to find unassigned work you can claim or tasks assigned to a specific agent. Project tasks include their trusted local path when available. Always use the task tools for changes; never edit tasks.json directly.",
        inputSchema: {
            type: "object",
            properties: {
                text: { type: "string" },
...
function renderTask(task) {
    ...
    if (task.dueAt)
        lines.push(`due: ${new Date(Number(task.dueAt)).toISOString()}`);
    if (task.description)
...
async function callTool(name, args) {
    ...
    if (name === "list_tasks") {
        ...
        for (const [source, target] of [
            ...
            if (args[source] !== undefined)
                query.set(target, String(args[source]));
        }
        const r = await api(`/api/internal/tasks${query.size ? `?${query}` : ""}`);
        const tasks = r.tasks ?? [];
        if (!tasks.length)
            return { text: "No tasks match those filters." };
        return { text: `Shared task board (${tasks.length}):\n\n${tasks.map(renderTask).join("\n\n---\n\n")}` };
    }
    if (name === "create_task") {
        if (!String(args.title ?? "").trim())
            return { text: "create_task needs a title.", isError: true };
    ...
    if (name === "claim_task") {
        ...
        if (!taskId || !Number.isInteger(revision))
            return { text: "claim_task needs task_id and revision.", isError: true };
    ...
    if (name === "update_task") {
        ...
        if (!taskId || !Number.isInteger(revision))
            return { text: "update_task needs task_id and revision.", isError: true };
        if (!Object.keys(patch).length)
            return { text: "update_task needs at least one field to change.", isError: true };
    ...
    switch (method) {
        case "initialize":
            ok(id, {
                ...
                capabilities: { tools: {} },
                serverInfo: { name: "gerer-build-studio-agents", version: "0.1.0" },
            });
            return;
