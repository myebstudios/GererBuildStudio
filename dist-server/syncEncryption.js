 ...
 */
export function isEncryptedPayload(val) {
...
export function deriveKey(passphrase, salt, iterations = DEFAULT_KDF_ITERATIONS) {
    ...
    return crypto.pbkdf2Sync(passphrase, salt, iterations, KEY_LENGTH_BYTES, "sha256");
}
/**
 * Async version of deriveKey using crypto.pbkdf2 for non-blocking operations if preferred
 */
export function deriveKeyAsync(passphrase, salt, iterations = DEFAULT_KDF_ITERATIONS) {
    return new Promise((resolve, reject) => {
        if (!passphrase || passphrase.trim().length === 0) {
            return reject(new Error("Passphrase cannot be empty"));
        }
        crypto.pbkdf2(passphrase, salt, iterations, KEY_LENGTH_BYTES, "sha256", (err, derivedKey) => {
            if (err)
                reject(err);
            else
                resolve(derivedKey);
 ...
 */
export function encryptSecrets(data, passphrase, iterations = DEFAULT_KDF_ITERATIONS) {
 ...
 */
export function decryptSecrets(payload, passphrase) {
    ...
    try {
        ...
        throw new Error("Failed to decrypt secrets: Invalid passphrase or corrupted ciphertext", { cause: err });
    }
}
/**
 * Specific helper for provider secrets map (e.g. { xai: "sk-...", composio: "ak-..." })
 */
export function encryptProviderSecrets(secrets, passphrase) {
    return encryptSecrets(secrets, passphrase);
}
/**
 * Specific helper for decrypting provider secrets map
 */
export function decryptProviderSecrets(payload, passphrase) {
    return decryptSecrets(payload, passphrase);
}
/**
 * Specific helper for instance driver config object
 */
export function encryptInstanceConfig(config, passphrase) {
    return encryptSecrets(config, passphrase);
}
/**
 * Specific helper for decrypting instance driver config object
 */
export function decryptInstanceConfig(payload, passphrase) {
    return decryptSecrets(payload, passphrase);
}
