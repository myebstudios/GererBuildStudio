# Contributing to Gerer Build Studio
...
Data lives in `~/.gerer-build-studio/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.gerer-build-studio`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.gerer-build-studio/config.json` via `PUT /api/config` and the API
