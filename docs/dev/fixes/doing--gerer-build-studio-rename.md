...
- [ ] **Replace internal abbreviations and public bridge names** — `electron/preload.cjs:1`, `src/types/ogb.d.ts:1`, `src/components/Composer.tsx:1`. Rename `window.ogb`, the declaration file, environment prefixes, local-storage keys, temporary paths, sockets, and MCP server identifiers to `gbs`/`GBS` equivalents.
